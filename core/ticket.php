...
if($_POST['modify']||$_POST['quit']||$_POST['mail']||$_POST['upload']||$save=="1"||$_POST['send']||$_POST['action']) 
{
	...
	////Generate Thread to technician and technician group attribution
		...
		if ($globalrow['technician']==0 && $_POST['technician']!='' && $globalrow['t_group']==0 && $globalrow['creator']!=$_SESSION['user_id'])
		{
    ...
    $row=mysql_fetch_array($query);
    if($_POST['state']!=3) $u_service=$row[0]; else $u_service=$globalrow['u_service'];
